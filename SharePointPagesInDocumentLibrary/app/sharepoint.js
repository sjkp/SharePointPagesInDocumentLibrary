var wopiframe = "https://sjkpdevs-my.sharepoint.com/personal/admin_sjkpdevs_onmicrosoft_com/_layouts/15/WopiFrame.aspx?sourcedoc={0}&action=interactivepreview&wdSmallView=1"

var getFiles = "https://sjkpdevs-my.sharepoint.com/personal/admin_sjkpdevs_onmicrosoft_com/_api/web/lists/GetByTitle('Test')/items?$expand=File"
...
var owa = "https://euc-word-view.officeapps.live.com/wv/docdatahandler.ashx?WOPIsrc=https%3A%2F%2Fsjkpdevs%2Dmy%2Esharepoint%2Ecom%2Fpersonal%2Fadmin%5Fsjkpdevs%5Fonmicrosoft%5Fcom%2F%5Fvti%5Fbin%2Fwopi%2Eashx%2Ffiles%2F{0}&access_token={1}&access_token_ttl={2}&type=png&o15=1&ui=en-US{3}"
...
$.ajax({
            ...
            success: function (res) {
                $.each(res.d.results, function (i, o) {
                    ...
                    $.get(wopiframe.replace('{0}', '{' + o.File.UniqueId + '}')).done(function (res) {
                        //console.log(res);
                        $html = $(res);
                        var accessToken = $('input[name="access_token"]', $html).val();
                        var accessTokenTTL = $('input[name="access_token_ttl"]', $html).val();

                        var owaUrl = owa.replace("{0}", o.File.UniqueId.replace(/-/g, ''));
                        ...
                        $.get(owaUrl).done(function (resw) {
                            ...
                            $('#pages').append('<li>' + o.File.Name + " " + noPages + '</li>');
                        });
                    });
                });
